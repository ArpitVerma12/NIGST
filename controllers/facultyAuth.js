...
exports.facultyCreation = async (req, res) => {
  ...
  try {
    ...
    const insertPasswordQuery =
      "INSERT INTO faculty_passwords(faculty_email, password) VALUES($1, $2)";
...
exports.facultyPassForgot = async (req, res) => {
  ...
  try {
    ...
    const checkQuery2 =
      "SELECT * FROM faculty_passwords WHERE faculty_email=$1";
    ...
    if (result1.rows.length === 0 || result2.rowCount === 0) {
      ...
      const updateQuery =
        "UPDATE faculty_passwords SET reset_token=$1 WHERE faculty_email=$2";
...
exports.facultyLogin = async (req, res) => {
  ...
  try {
    ...
    const decryptedEmail = CryptoJS.AES.decrypt(email, keyDecription).toString(
      CryptoJS.enc.Utf8
    );
    ...
    const passwordQuery =
      "SELECT * FROM faculty_passwords WHERE faculty_email = $1";
    const passwordResult = await client.query(passwordQuery, [user.email]);
...
exports.fPassReset = async (req, res) => {
  ...
  try {
    ...
    const updatePassword =
      "UPDATE faculty_passwords SET password = $1, reset_token = NULL WHERE faculty_email = $2";
...
exports.fChangePassword = async (req, res) => {
  ...
  try {
    ...
    const updatePassword =
      "UPDATE faculty_passwords SET password = $1 WHERE faculty_email = $2";
...
exports.facultyPosition = async (req, res) => {
  ...
  try {
    ...
    const checkResult1 = await pool.query(
      "SELECT * FROM faculty_position WHERE faculty_pos = $1",
      [faculty_pos]
    );
    ...
    const create =
      "INSERT INTO faculty_position(faculty_pos, position_id, description) VALUES ($1,$2,$3) ";
    await client.query(create, [...data]);
...
exports.positionSend = async (req, res) => {
  ...
  try {
    ...
    const query1 =
      "SELECT first_name, middle_name, last_name, designation FROM faculty";
...
exports.facultyPositionAssi = async (req, res) => {
  ...
  try {
    ...
    const { facultyId, faculty_pos, faculty_admin, position_assi_id } =
      req.body;
    ...
    if (resultAdmin.rowCount === 0) {
      return res
        .status(400)
        .send({ message: "Admin of this faculty does not exist." });
    }
    ...
    if (resultPosition.rowCount === 0) {
      return res
        .status(400)
        .send({ message: "Faculty position does not exist." });
    }

    const query =
      "INSERT INTO faculty_position_assi (faculty_id, faculty_pos, faculty_admin, position_assi_id) VALUES ($1, $2, $3, $4)";
    ...
    return res
      .status(200)
      .send({ message: "Successfully assigned faculty position." });
  } catch (error) {
    console.error(error);
    ...
    if (client) {
      try {
        ...
        console.error(
          "Error occurred while releasing the database client:",
          releaseError
        );
      }
    }
...
exports.facultyPositionReAssign = async (req, res) => {
  ...
  try {
    ...
    const updateQuery =
      "UPDATE faculty_position_assi SET faculty_pos=$1,position_assi_id=$2,faculty_admin=$3,updated_At=$4 WHERE faculty_id=$5";
...
exports.viewFaculty = async (req, res) => {
  ...
  try {
    ...
    const dataQ =
      "SELECT * FROM faculty ORDER BY created_on_date_time LIMIT $1 OFFSET $2";
...
exports.reportSubmission = async (req, res) => {
  ...
  try {
    ...
    const [result2, facultyResult, result] = await Promise.all([
      client.query(
        "SELECT cs.course_id, cs.course_status, c.course_officer FROM course_scheduler cs INNER JOIN courses c ON c.course_id = cs.course_id WHERE cs.course_scheduler_id = $1",
        [scheduleId]
      ),
      client.query("SELECT * from faculty_name WHERE name = $1", [faculty]),
      client.query(
        "SELECT * FROM report_submission WHERE faculty_id = $1 AND schedule_id = $2",
        [facultyId, scheduleId]
      ),
    ]);
    ...
    const { course_status: courseStatus, course_officer: courseOfficer } =
      result2.rows[0];

    if (courseOfficer !== facultyId) {
      return res
        .status(403)
        .send({ message: "You are not the Course Officer for this course!" });
    }
    ...
    if (courseStatus !== "completed") {
      return res.status(400).send({
        message:
          "You are not allowed to submit a report for a course that is not completed!",
      });
    }
    ...
    await client.query(
      "INSERT INTO report_submission (faculty_id, schedule_id, report_path, remarks, faculty) VALUES ($1, $2, $3, $4, $5)",
      [facultyId, scheduleId, reportPath, remarks, faculty]
    );
...
exports.displayReport = async (req, res) => {
  ...
  try {
    ...
    const query =
      "SELECT report_path FROM report_submission WHERE schedule_id = $1";
    ...
    if (result.rowCount === 0) {
      return res
        .status(404)
        .send({ message: "No report found for the specified schedule ID!" });
    }
...
exports.sendIDForReport = async (req, res) => {
  ...
  try {
    ...
    const check =
      "SELECT title as name, course_id  FROM courses WHERE faculty = $1";
    ...
    if (result01.rowCount === 0) {
      return res
        .status(404)
        .send({ message: `No Course Found with faculty: ${faculty}` });
    } else {
      ...
      const coursePromises = courses.map(async (course) => {
        ...
        const check01 =
          "SELECT name, course_scheduler_id as schedulerId,batch_no as batch from course_scheduler WHERE course_id = $1 AND course_status = $2";
      ...
      if (finalData.length === 0) {
        return res
          .status(404)
          .send({ message: "No completed courses found for this faculty." });
      } else {
